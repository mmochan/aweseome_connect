   ...
   binding.pry
    ...
    if signed_in?
      if @identity.user == current_user
        ...
        AppdotnetSocial.find_or_create_by_uid_and_username_and_identity_id(auth.uid, auth.info.nickname, @identity)
        redirect_to :back, notice: "Already linked that account!"
      else
        ...
        @identity.update_attributes(oauth_token: auth['credentials']['token'], oauth_expires_at: auth['credentials']['expires_at'])        
        AppdotnetSocial.find_or_create_by_uid_and_username_and_identity_id(auth.uid, auth.info.nickname, @identity)
        redirect_to :back, notice: "Successfully linked that account!"
      end
    else
